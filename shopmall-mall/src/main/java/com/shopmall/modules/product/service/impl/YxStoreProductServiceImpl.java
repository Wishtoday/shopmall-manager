...
import com.shopmall.modules.product.dto.*;
import com.shopmall.modules.product.mapper.StoreProductMapper;
...
//        return resultMap;
    ...
    private ProductResultDto computedProduct(List<ProductFormatDto> attrs) {
        ...
        if (stock <= 0) {
            throw new ShopMallException("库存不能低于0");
        }
    ...
    private DetailDto attrFormat(List<FromatDetailDto> fromatDetailDTOList) {
        ...
        fromatDetailDTOList.stream()
                ...
                .forEach(i -> {
                    if (i == null || i.isEmpty()) {
                        throw new ShopMallException("请至少添加一个规格值哦");
                    }
                    String str = ArrayUtil.join(i.toArray(), ",");
                    if (str.contains("-")) {
                        throw new ShopMallException("规格值里包含'-',请重新添加");
                    }
                });
